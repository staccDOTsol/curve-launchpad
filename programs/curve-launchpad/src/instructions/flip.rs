...
pub fn flip(ctx: Context<Flip>, wager_meme_amount: u64) -> Result<()> {
    ...
    // ------------------------------------------------------------------
    let clock = Clock::get()?;
    // Switchboard's API expects solana_clock::Clock (2.x); anchor 0.30.1
    // hands us solana_program::clock::Clock (1.18.x). Same fields, distinct
    // types in the type system. Build the 2.x value from the 1.x read.
    let sb_clock = solana_clock::Clock {
        slot: clock.slot,
